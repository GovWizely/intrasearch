...
RSpec.configure do |config|
  config.before(:suite) do
    Elasticsearch::Persistence::Repository.new.client.indices.delete index: "nix-#{Nix.env}-*"
  ...
  before(:all) do
    shared_model_classes.each do |model_class|
      IndexManager.new(model_class).setup_new_index! do
        ...
        yaml = YAML.load Nix.root.join("spec/fixtures/yaml/#{model_source_filename}.yml").read
